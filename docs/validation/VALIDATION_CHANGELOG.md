...
- **MCE-4b — Premiership finals AB-subsequence order was still unenforced for SSP (reopen of MCE-4):**
  - MCE-4 added the `getAbPrSourceColIdx` resolver to `validateBestHairPROrder`, but the **call site never reached it for SSP**. PR dispatched the LH/SH order check as an either/or on `column.specialty` — `'Longhair'/'Shorthair'` → top-15 self-check only; `'Allbreed'` → AB-subsequence check. An SSP ring's Best LH/SH PR finals live in `'Longhair'/'Shorthair'` columns, so the AB-subsequence check (`validateBestHairPROrder`) was never invoked. The resolver fix was dead code for SSP.
  - **Fix:** made the PR dispatch **additive** (mirrors Championship, which already runs `validateBestHairCHOrder` unconditionally for LH/SH): the top-15 single-specialty check and the AB-subsequence check are now independent, each guarded by its own precondition. `validateBestHairPROrder` runs for LH/SH regardless of specialty; it self-no-ops for a standalone specialty ring (no AB sibling → empty AB list). Root cause was a pre-SSP "specialty column ⇒ standalone ring" assumption baked into the either/or; SSP introduced columns that are simultaneously a specialty column **and** part of an allbreed ring.
  - The MCE-4 PR test passed for the wrong reason (a different rule — show-awards cross-column order-preservation — emitted a matching "out of order" string). Replaced with **cell-specific** assertions that isolate the finals AB-subsequence rule by cell key + wording.
- **MCE-5 — cross-column finals duplicate (cat in both Best LH and Best SH) now caught for SSP (CH + PR):**
  - The same-column "cannot be both longhair and shorthair" finals check compares `lh*Finals[colIdx]` vs `sh*Finals[colIdx]` at the **same** column index, so it no-ops for SSP (Best LH and Best SH finals are in different columns). Added `validateSpecialtyFinalsCrossColumnDuplicatesCH`/`PR` to the SSP cross-column suite (step 6) to re-run the check across the split LH/SH columns. This closes the gap previously left as "indirectly covered / intentionally skipped".
- **Duplicate message unification:** all finals-section "cannot be both longhair and shorthair" messages (same-column + MCE-5 cross-column, CH + PR) now name the cat — `Duplicate: Cat #<n> cannot be both longhair and shorthair` — matching the Show Awards section's existing wording (previously the finals sections used a generic "a cat …" with no cat number).
- **Files:** `src/validation/championshipValidation.ts`, `src/validation/premiershipValidation.ts`
- **Tests:** `src/validation/sspReminder.test.ts` reworked — rule-specific + cell-specific assertions; coverage indexed by error-class × partition (subsequence / filler / valid × SSP-LH / SSP-SH / plain-Allbreed / standalone / 5-position); PR brought to Championship parity. Verified the order tests **fail on the pre-MCE-4b code** (3 SSP cases) and pass after — proving they catch the regression. 74 tests pass; `vite build` clean; lint at baseline.
